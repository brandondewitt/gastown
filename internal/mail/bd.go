...
// NOTE: --no-daemon is prepended to prevent API calls from hanging when
// the bd daemon isn't running (web dashboard, etc.)
func runBdCommand(args []string, workDir, beadsDir string, extraEnv ...string) ([]byte, error) {
	// Prepend --no-daemon to prevent hangs when daemon isn't running
