    ...
    {:else if status}
      ...
      {#if status.agents && status.agents.length > 0}
        <section class="section">
          <h2 class="section-header">Agents</h2>
          <div class="agent-grid">
            {#each status.agents as agent}
              {@const roleColor = getRoleColor(agent.role)}
              <div class="agent-button" style="border-color: {roleColor}">
                <span class="agent-icon" style="color: {roleColor}">
              ...
              {#if rig.agents && rig.agents.length > 0}
                <div class="rig-agents">
                  {#each rig.agents as agent}
                    {@const roleColor = getRoleColor(agent.role)}
                    <div class="agent-row">
                      <span class="agent-icon small" style="color: {roleColor}">
                        {getRoleIcon(agent.role)}
                      </span>
                      <span class="agent-name">{agent.name}</span>
